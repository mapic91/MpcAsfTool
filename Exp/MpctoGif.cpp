...
bool CovMpctoGif::Save(FileType type)
{
    ...
    else
    {
        ...
        if(type == T_GIF)
        {
            ...
            {
                strdata = decode.GetDecodedFrameData(i, (long*)&width, (long*)&height, MpcDecode::MPC_RGB, &istran);
                if(!strdata)
                {
                    wxLogDebug(wxT("Failed at - %s: %d"), __FILE__, __LINE__);
                    return false;
        ...
        else if(type == T_PNG)
        {
            ...
            for(unsigned long i = 0; i < framessum; i++)
            {
                strdata = decode.GetDecodedFrameData(i, (long*)&width, (long*)&height, MpcDecode::MPC_BGRA, &istran);
                if(!strdata)
                {
                    wxLogDebug(wxT("Failed at - %s: %d"), __FILE__, __LINE__);
                    return false;
                ...
                FreeImage_SaveU(FIF_PNG, fibmp,
                                (outFilePath+wxFileName::GetPathSeparator()+
                                 infilename+wxString::Format(wxT("-frame%03ld"), i+1)+
                                 wxT(".png")).c_str(), 0);
