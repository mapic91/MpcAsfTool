...
bool WorkManager::OpenFile(wxString InPath)
{
    ...
    if(ext.CmpNoCase(wxT("mpc")) == 0)
    {
        ...
        shdpath = wxFileName::FileName(InPath).GetPath(wxPATH_GET_SEPARATOR|wxPATH_GET_SEPARATOR);
        ...
        tempshddata = NULL;
        for(unsigned long conti = 0; conti < mpcdecode.GetFramesCounts(); conti++)
        {
    ...
    else if(ext.CmpNoCase(wxT("asf")) == 0)
    {
        ...
        for(unsigned long conti = 0; conti < asfdecode.GetFramesCounts(); conti++)
        {
...
FILOCRGBQUAD* WorkManager::GetUndeletedGlobalizedFrameData(unsigned long index)
{
    ...
    for(long hi = 0; hi < GetGlobalHeight(); hi++)
    {
        for(long wi = 0; wi < GetGlobalWidth(); wi++)
        {
            ...
            {
                glbdata[hi*GetGlobalWidth()+wi].rgbRed = decdata[hi*width+wi].rgbRed;
                glbdata[hi*GetGlobalWidth()+wi].rgbGreen = decdata[hi*width+wi].rgbGreen;
                glbdata[hi*GetGlobalWidth()+wi].rgbBlue = decdata[hi*width+wi].rgbBlue;
                glbdata[hi*GetGlobalWidth()+wi].rgbReserved = decdata[hi*width+wi].rgbReserved;
            }
        }
    }
...
wxImage WorkManager::GetUndeletedGlobalizedImage(unsigned long index)
{
    ...
    if(shdincluded && !makeshadow && shdframe != NULL)
    {
...
bool WorkManager::SaveToMpc(const wxString outpath)
{
    ...
    if(makeshadow)
    {
        ...
        wxString shdpath;
        shdpath = wxFileName::FileName(outpath).GetPath(wxPATH_GET_SEPARATOR|wxPATH_GET_SEPARATOR);
...
bool WorkManager::SaveToGif(const wxString outpath)
{
    ...
    else
    {
        ...
        if(alphamask != 0)
        {
            shdproc.MakeTransMask(alphamask);
        }
