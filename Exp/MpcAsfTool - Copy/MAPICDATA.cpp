...
void MAPICDATA::AddBaseColor(FILOCRGBQUAD basecolor)
{
    ...
    for(unsigned long frmi = 0; frmi < framecounts; frmi++)
    {
        for(long pixi = 0; pixi < globalwidth*globalheight; pixi++)
        {
            ...
            tempframe->data[pixi].rgbRed   = (BYTE)(((double)tempframe->data[pixi].rgbRed)*((double)alpha/(double)0xFF) +
                                                    ((double)basecolor.rgbRed)*((double)(0xFF-alpha)/(double)0xFF));
            tempframe->data[pixi].rgbGreen = (BYTE)(((double)tempframe->data[pixi].rgbGreen)*((double)alpha/(double)0xFF) +
                                                    ((double)basecolor.rgbGreen)*((double)(0xFF-alpha)/(double)0xFF));
            tempframe->data[pixi].rgbBlue  = (BYTE)(((double)tempframe->data[pixi].rgbBlue)*((double)alpha/(double)0xFF) +
