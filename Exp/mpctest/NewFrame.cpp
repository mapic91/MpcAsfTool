...
void NewFrame::OnButton_mpcfileClick(wxCommandEvent& event)
{
    ...
    {
        currentframe = 0;
        if(mpcdecode.ReadMpcFile(filedlg.GetPath()))
        {
            bitmap_view->SetBitmap(wxBitmap(mpcdecode.GetFrameImage(currentframe)));
            this->SetTitle(mpcdecode.GetVersionInfo() +
                           wxT("  Frames:") +
                           wxString::Format(wxT("%ld"), mpcdecode.GetFramesCounts()) +
                           wxT("  Colour:") +
                           wxString::Format(wxT("%ld"), mpcdecode.GetColourCounts()));
...
{
    if(!mpcdecode.GetFilePath().IsEmpty() && currentframe != 0)
    {
        currentframe--;
        bitmap_view->SetBitmap(wxBitmap(mpcdecode.GetFrameImage(currentframe)));
        StaticText_current_pic->SetLabel(wxString::Format(wxT("%d"), currentframe+1));
...
{
    if(!mpcdecode.GetFilePath().IsEmpty() && currentframe+1 != mpcdecode.GetFramesCounts())
    {
        currentframe++;
        bitmap_view->SetBitmap(wxBitmap(mpcdecode.GetFrameImage(currentframe)));
        StaticText_current_pic->SetLabel(wxString::Format(wxT("%d"), currentframe+1));
