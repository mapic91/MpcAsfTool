...
bool GifData::FilePaletteLoctoGlb(const wxString inpath, long Interval)
{
    ...
    if(!gifmem)
    {
        //wxMessageBox(wxT("Open memory failed"));
        return false;
    }
    ...
    if(!glbpal)
    {
        //wxMessageBox(wxT("Convert to memory failed"));
        return false;
    }
    res = FreeImage_AcquireMemory(glbpal, &buffer, &bufsize);
    if(!res)
    {
        //wxMessageBox(wxT("Acquire memory failed"));
    ...
    if(!outfile.is_open())
    {
        //wxMessageBox(wxT("OutPut file open failed"));
        FreeImage_CloseMemory(glbpal);
        return false;
    }
